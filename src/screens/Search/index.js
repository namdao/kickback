...
import {HOME, SEARCH_RESULT, FollowerHistory} from "../../actions/follower.history";
...
export default class Search extends React.Component {
  ...
  componentWillMount = () => {
    ...
    if (this.screen === HOME && this.props.searchMode ==="basic"){
  ...
  showBox = () => {
    NavigationActions.pop();
    NavigationActions.navigateToSearchAdvance({ fromScreen: this.props.fromScreen, textSearch: this.changeTextSearch, searchMode: "advance"});
  }
  ...
  componentDidUpdate(prevProps) {
    if (this.props.searchReducer.data != prevProps.searchReducer.data) {
      if (this.props.searchReducer.count == this.props.searchReducer.data.length) {
        this.setState({ loadmore: false })
      }
    }
  }
  renderFooter = () => {
    if (!this.props.loadingReducer.isLoadMoreSearch) return null;
    return (
  ...
  _onEndReached = async () => {
    if (!this.onEndReachedCalledDuringMomentum && !this.props.loadingReducer.isLoadMoreSearch) {
      this.onEndReachedCalledDuringMomentum = true;
      const { data, count, next } = this.props.searchReducer;
  ...
  render() {
    ...
    const dataList = this.props.searchReducer.data;
    return (
      <Container>
        ...
        /> : null}
        {!loadingReducer.isLoading && isDisplay && this.props.searchReducer.data.length == 0 ? <TouchableOpacity
