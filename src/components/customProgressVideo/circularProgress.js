...
import SHAPE_CIRCLE from '../../thumbnails/circular_color.png';
...
export default class CircularProgress extends Component {
  ...
  extractFill() {
    return Math.min(300, Math.max(0, this.props.fill));
  }

  renderBackgroundPath() {
    const {size, width, backgroundColor, r1, r2} = this.props;
    ...
    // />
        <Image resizeMode={'cover'} source={SHAPE_CIRCLE} style={{width: r2*2, position: 'absolute', height: r2*2}}/>
    );
  }
  ...
  renderCirclePaths() {
    ...
    let numberOfPathsToDraw = Math.floor(
      ((2 * Math.PI) * (fill / 300)) / ((2 * Math.PI) / noOfSeg)
    );
    let rem = (((2 * Math.PI) * (fill / 300)) / ((2 * Math.PI) / noOfSeg)) % 1;
    if(rem > 0) {
      numberOfPathsToDraw++;
    }
    let startAngle = 0;
